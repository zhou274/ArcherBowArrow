...
public class GameController : MonoBehaviour {
        ...
        ShowVideoAd("192if3b93qo6991ed0",
            (bol) => {
                if (bol)
                {
                    ...
                    player.GetComponent<PlayerController>().playerCurrentHealth += 50;
	...
	IEnumerator finishTheGame(int res) {
		...
		//set the label
		if (res == 0) {
			uiGameStateLabel.GetComponent<TextMeshPro> ().text = "游戏失败 :(";
		} else if (res == 1) {
			uiGameStateLabel.GetComponent<TextMeshPro> ().text = "游戏成功";
		} else if (res == 2) {
			uiGameStateLabel.GetComponent<TextMeshPro> ().text = "狩猎愉快吗？";
			//set stat info
